...
*/
	...
	public function getPolarization($module)
	{
	...
	public	function getPolarizationData($module, $chr, $row)
		{
			$c = 0;
			$num = 81;
		 	$trashold = -0.8; //trashhold
		 	//if($row ==  0)$row = 1;
		 	$gen = array();
		 	
			$q = "SELECT * FROM ".$this->module( $module ).".ind WHERE nrow = ".$row." AND chr = ".$chr.";";
			$stmt = $this->db->prepare($q);
			$stmt->execute();
			...
			else $bottomlimit =  " t1.ind - $num ";
			
		  	
		  	$query = "SELECT * FROM ".$this->module( $module ).".gen g WHERE g.ind = ".$ind .";";
			$stmt = $this->db->prepare($query);
			$stmt->execute();
			$md = $stmt->fetch(\PDO::FETCH_ASSOC);
				$current = $md['gen'];
		  	...
		  	$query = "SELECT * FROM ".$this->module( $module ).".gen g WHERE g.ind < ". $t ." AND g.ind >  ". $b  .";";
		 
			$query1 = "SELECT 
				CASE
					WHEN t3.lde <= $t then 1
					WHEN t3.lde > ABS($t) then -1
					WHEN t3.lde IS NULL then 0
				END AS LD 
			FROM  ".$this->module( $module ).".ind t1 JOIN ".$this->module( $module ).".ind t2 
			LEFT JOIN ".$this->module( $module ).".lde t3 
			ON t3.ind_a = t1.ind AND t3.ind_b=t2.ind  
			WHERE t1.ind = ".$ind." AND t2.ind < t1.ind + $num AND t2.ind > $bottomlimit AND t1.chr = ".$chr." AND t2.chr = ".$chr."
			ORDER BY t2.ind ;";
			...
			while ($md = $stmt->fetch(\PDO::FETCH_ASSOC)){
				...
				$ld = 0;
				$ld = $this->lde_hwe($current, $md['gen'],  $num * 2 - 1, '', '',10000);
