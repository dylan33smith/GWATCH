...
if(m_ColumnData[i]["Name"].indexOf('Density') != -1) 
    ...
    var color = [red,green,blue];	
        ...
        //Track of genes
			...
			//if(showTrack == 1){
			if(m_ColumnData[i]["Name"].indexOf('Genes') != -1){
				
				//right = startX  ;
				//left = right + 2*halfColumnSize;
			
				right = right + 4
				left = left + 2 //numColumns
				height = 1;
				if(oddsRatio == -1){
				color = [.5,.3,.1]; 
				}
		...
		// End track of genes
            ...
            //front
            vertsFloatArray.addData3(left, -1.0, front);
            vertsFloatArray.addDataList(color);

            vertsFloatArray.addData3(right, -1.0, front);
            vertsFloatArray.addDataList(color);
            ...
            vertsFloatArray.addData3(left, -1.0, back);
            vertsFloatArray.addDataList(color);

            vertsFloatArray.addData3(right, -1.0, back);
            vertsFloatArray.addDataList(color);
...
RowCache.prototype.getDescriptiveText = function (offset, eyePos, mouseDir, invert) {
    ...
    for (var row = 0; row < numRows; ++row) {
		...
		var compare = '';//"<br><span id=\"compare_link\"><a href=\"#\" onClick = \"fill('/compare?module=" + m_module + "&pos=" + this.data["Rows"][row]["Coords"]  + "&chr=" + m_chr + "', 'compare_link')\"> Generate Link </a></span>"
 		...
 		{ 
                ...
                our_string = "<a href=\"/report.php?format=svg&reportType=trax&row=" +  this.data["Rows"][row]["nrow"] + "&pos=" +  this.data["Rows"][row]["Coords"] + "&chr=" + m_chr + "&module=" + m_module + "\" target=_blank>TRAX " + trax + " </a>";
                rs = rs.replace(/-1/, '')
                var statname = 'QSS';

 /////
                if (m_ColumnData[i]["StatName"] != '') statname = m_ColumnData[i]["StatName"];
                var descString =  "<br>SNP rs ID: " + this.data["Rows"][row]["nrow"] + " "+ this.data["Rows"][row]["Name"];
             	descString += compare;
                ...
                descString += "<a href=\"/" + m_module + "/mplot/" + i +"\" target=\"_blank\">" + m_ColumnData[i]["Name"] + "</a><br/>";
